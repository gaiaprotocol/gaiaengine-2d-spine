...
export default class Spine extends GameObject {
    ...
    async load() {
        ...
        const promises = [];
        promises.push((async () => textAtlasData = await (await fetch(this.options.atlas)).text())());
        if (this.options.skel !== undefined) {
            promises.push((async () => skeletonBynary = new Uint8Array(await (await fetch(this.options.skel)).arrayBuffer()))());
        }
        else if (this.options.json !== undefined) {
            promises.push((async () => textSkeletonData = await (await fetch(this.options.json)).text())());
        }
        else {
